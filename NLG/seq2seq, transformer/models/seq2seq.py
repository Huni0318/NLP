...
class Attention(nn.Module):
    ...
    def forward(self, h_src, h_t_tgt, mask=None):
        ...
        query = self.linear(h_t_tgt) # h_t_tgt
        # |query| = (batch_size, 1, hidden_size)

        weight = torch.bmm(query, h_src.transpose(1, 2)) # query x key, h_src = value
        ...
        # |context_vector| = (batch_size, 1, hidden_size) , c = w x v, |w| = (bs, 1, n), |v| = (bs, n, hs)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        if isinstance(src, tuple):
            x, x_length = src # |src| = (bs, n) ~ (bs, n, |v_src|)
